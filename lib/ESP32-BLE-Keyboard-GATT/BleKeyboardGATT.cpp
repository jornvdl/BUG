...
/// Flags and values for communication over custom BLE characteristic
// KEYBIND CHARACTERISTIC
int keystroke;  // integer value corresponding to keybind

// INDICATOR CHARACTERISTIC
int colour[3];  // RGB colour value of the LEDs: each int in range 0x00~0xFF
...
bool rstTimer;	// general flag to reset timer when ANY characteristic is updated

// INFO CHARACTERISTIC
bool rstBUG;	 	// flag: reset to factory settings
bool identify; 	// flag: flash indicators to identify BUG
bool wasd;			// flag: when circulating, use WASD keybinds instead of arrows
bool keys[5];		// flag-array: select which keys to circulate. When all are zero, a custom key should be set.
...
class stateCallbacks: public BLECharacteristicCallbacks {
  void onWrite(BLECharacteristic *stateCharacteristic) {
    ...
    wasd 			= (*data & 0x20) == 0x20;
    keys[4]		= (*data & 0x10) == 0x10;
    keys[0]		=	(*data & 0x08) == 0x08;
...
int* BleKeyboard::getKeybind() { // not needed when using hardcoded keys in lib
	return &keystroke;
}
...
void BleKeyboard::setKeybind(int* k) {
	keystroke = *k;
	keyCharacteristic->setValue(keystroke);
}
...
// Share and reset timer flag
bool* BleKeyboard::flgRstTimer() {
	return &rstTimer;
}
